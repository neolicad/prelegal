...
export function renderDocument(
  ...
  if (spec.renderer === "nda-coverpage" && templates.coverPage) {
    const coverPage = renderNdaCoverPage(templates.coverPage, spec, values);
    return `${coverPage}\n\n---\n\n${standardTerms}`;
  }
  return `${renderKeyTermsBlock(spec, values)}\n\n---\n\n${standardTerms}`;
}
