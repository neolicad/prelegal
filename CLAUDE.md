...
- **PL-10** (done): real signup/login/logout, per-user document history, and a visual polish pass across every screen.
  ...
  - Follow-up bug found during manual testing of the above: sending a chat message while scrolled down to reach the input box (e.g. on the document workspace, where the chat panel sits above it) could snap the page back up. Root cause: `ChatMessageList`'s auto-scroll inferred whether to scroll its own container vs. the whole page from *current* content overflow (`scrollHeight > clientHeight`) rather than the container's actual layout -- a bounded, fixed-height panel that simply hadn't filled its height yet (e.g. early in a short conversation) was misread as the picker's unbounded page-level chat, falling back to `scrollIntoView` on the whole page. Fixed with an explicit `bounded` prop instead of inferring from current fill state; reproduced against a real browser via Playwright before fixing.
  - Second follow-up: the document workspace's middle "Key Terms" column (`DocumentForm`) previously had no height limit of its own, so reaching a field below the fold meant scrolling the whole page. It now scrolls independently (`max-h-[calc(100vh-10rem)] overflow-y-auto`, matching the preview column's existing treatment), and `DocumentForm` accepts a `scrollTo` request -- keyed by field/party key plus a bump counter so the same field can be re-targeted on consecutive turns -- that `DocumentApp` fires from `DocumentChatPanel`'s new `onFieldsUpdated` callback whenever a chat turn actually fills in a field, so the column scrolls to (and centers) whatever the AI just answered.
  - Third follow-up, found via manual testing of the above: a clarifying-question turn that filled nothing in (e.g. the AI asking "What's the Effective Date?") could still scroll to the wrong place -- to Party 1, nowhere near the field being discussed. Root cause: a party update is an object (`{printName: "", title: "", ...}`), which is truthy on its own even when every subfield in it is empty, so the naive "was some key in `updates` truthy" check picked it as "the updated field" regardless. Fixed with `document-chat.ts`'s `isMeaningfulFieldUpdate`, which checks a party update subfield-by-subfield the same way `mergeParty` already does, so "was this key updated" agrees with "did merging it actually change anything"; reproduced by temporarily reverting the fix and confirming a new regression test failed exactly this way before fixing it back.